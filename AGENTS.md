...
A compiled static website documenting the chronology of the **Knights of
Columbus** — the Catholic fraternal benefit society founded in New Haven in
1882 — including, as a declared thread, the documented conflict between the
Order and organized anti-Catholicism in the United States (the fabricated
"bogus oath" of 1912, the second Ku Klux Klan, the Oregon School Law fight).
...
Non-negotiable posture for this subject:

- **The "bogus oath" is documented as a fabrication** — that finding belongs
  to the U.S. House of Representatives (1913 election-case report) and to the
  courts that convicted its publishers of criminal libel. Cite the acts, never
  reproduce the slur as if it were a disputed document.
- **Characterizations of the Ku Klux Klan and of the Order are attributed** to
  the historians and period documents that make them; the site's own voice
  takes no side. This includes the Order's self-crediting claims (fund
  figures, legislative credit, martyr membership), which are recorded as the
  Order's own account when no independent source corroborates them.
- **The race-exclusion thread is recorded, not adjudicated:** the founding
  context of the Knights of Peter Claver (1909) is told differently by
  different sources, and the dataset records the competing framings.
- **Scale figures are time-stamped** (membership, insurance in force,
  charitable giving change every year — state the as-of date).
- Fr. McGivney's beatification is an act about the man, not about the Order;
  keep person-judgments and organization-history distinct.
